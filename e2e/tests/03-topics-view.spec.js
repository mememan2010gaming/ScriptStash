...
test.describe('Topics view', () => {
  ...
  test('searching for non-existent term shows empty or filtered state', async ({ page }) => {
    ...
    await Promise.race([
      ...
      page.waitForFunction(() => document.querySelectorAll('[data-card]').length === 0, { timeout: 8_000 }),
    ]).catch(() => {});
    const cards = page.locator('[data-card]');
  ...
  test('loading skeleton is shown (and replaced) on initial load', async ({ page }) => {
    await page.waitForSelector('[class*="skeleton"], [class*="Skeleton"]', { state: 'hidden', timeout: 8_000 }).catch(() => {});
