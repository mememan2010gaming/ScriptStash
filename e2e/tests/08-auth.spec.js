...
const { MOCK_USER } = require('../fixtures/mock-data');
...
test.describe('Auth & session', () => {
  ...
  test('user info is displayed in sidebar', async ({ page }) => {
    ...
    const userText = page.locator('aside').getByText(/TestUser|Guest|logged/i).first();
  ...
  test('logout IPC is called when logout button clicked', async ({ page, electronApp }) => {
    ...
    await electronApp.evaluate(({ ipcMain }) => {
      ...
      ipcMain.on('logout', () => { global.__logoutReceived = true; });
    });
