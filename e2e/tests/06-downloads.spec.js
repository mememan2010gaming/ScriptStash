...
test.describe('Downloads view', () => {
  ...
  test('active download items are shown when in progress', async ({ page, electronApp }) => {
    ...
    const appeared = await item.first().waitFor({ state: 'visible', timeout: 3_000 }).then(() => true).catch(() => false);
  ...
  test('progress bar renders for active download', async ({ page, electronApp }) => {
    ...
    const appeared = await filename.first().waitFor({ state: 'visible', timeout: 3_000 }).then(() => true).catch(() => false);
    if (!appeared) { test.skip(); return; }
    const progress = page.locator('[role="progressbar"]').or(page.getByText(/65%/));
    const hasProgress = await progress.first().waitFor({ state: 'visible', timeout: 2_000 }).then(() => true).catch(() => false);
