...
test.describe('Density mode', () => {
  async function goToAppearance(page) {
    ...
    await expect(
      page
        ...
        .filter({ hasText: /mosaic|list|compact/i })
        .first()
  ...
  test('each density card has a description', async ({ page }) => {
    await goToAppearance(page)
    await expect(page.getByText(/thumbnail tile grid|grid of tiles/i)).toBeVisible({
      timeout: 8_000,
    })
    await expect(page.getByText(/wide rows with preview|rows with preview/i)).toBeVisible({
      timeout: 8_000,
    })
  ...
  for (const mode of ['mosaic', 'list', 'compact']) {
    test(`clicking ${mode.charAt(0).toUpperCase() + mode.slice(1)} card saves to localStorage`, async ({
      page,
    }) => {
      await goToAppearance(page)
      const card = densityCards(page)
      ...
      await card.click()
      await page.waitForFunction(m => localStorage.getItem('ss_density') === m, mode, {
        timeout: 3_000,
      })
  ...
  test('density setting persists across navigation', async ({ page }) => {
    await goToAppearance(page)
    const compactCard = densityCards(page)
      .filter({ hasText: /compact/i })
      .first()
    ...
    await page.waitForFunction(() => localStorage.getItem('ss_density') === 'compact', {
      timeout: 3_000,
    })
  ...
  test('density setting persists across page reload', async ({ page }) => {
    await goToAppearance(page)
    const mosaicCard = densityCards(page)
      .filter({ hasText: /mosaic/i })
      .first()
    ...
    await page.waitForFunction(() => localStorage.getItem('ss_density') === 'mosaic', {
      timeout: 3_000,
    })
  ...
  test('clicking different density card updates selection', async ({ page }) => {
    ...
    await page.waitForFunction(() => localStorage.getItem('ss_density') === 'list', {
      timeout: 3_000,
    })
