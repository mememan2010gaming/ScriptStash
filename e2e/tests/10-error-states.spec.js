...
test.describe('Error states & resilience', () => {
  test('topics view shows empty state when API returns empty list', async ({ page, electronApp }) => {
  ...
  test('search handles network error gracefully', async ({ page, electronApp }) => {
    ...
    const nav = page.locator('nav').first();
    await nav.getByText(/search/i).first().click();
    await page.waitForTimeout(300);

    const input = page.getByPlaceholder(/Search scripts/i).or(page.getByRole('searchbox')).first();
  ...
  test('topic detail handles missing topic gracefully', async ({ page, electronApp }) => {
    await electronApp.evaluate(({ ipcMain }) => {
      ...
      ipcMain.handle('get-topic-details', () => ({ success: false, error: 'Topic not found', status: 404 }));
    });
  ...
  test('app recovers when IPC handler throws unexpectedly', async ({ page, electronApp }) => {
    await electronApp.evaluate(({ ipcMain }) => {
      ipcMain.removeHandler('get-topics');
      ipcMain.handle('get-topics', () => { throw new Error('Unexpected crash'); });
    });
  ...
  test('toast appears on yt-dlp update failure', async ({ page, electronApp }) => {
    ...
    const dlBtn = page.locator('main').getByRole('button', { name: /^downloads$/i }).first();
