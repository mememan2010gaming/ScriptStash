...
const { MOCK_DOWNLOAD_PATH, MOCK_APP_VERSION, MOCK_YTDLP_VERSION } = require('../fixtures/mock-data');
...
test.describe('Settings view', () => {
  ...
  test('settings section navigation is visible', async ({ page }) => {
    ...
    const sectionNav = page.getByText(/general/i)
  ...
  test('clicking a theme card visually selects it', async ({ page }) => {
    ...
    // Theme buttons use class "glass glass-hover"; they always have theme name text
    const themes = page.locator('[class*="glass-hover"]').filter({ hasText: /[a-z]{3,}/i });
    if (await themes.count() < 2) { test.skip(); return; }
  ...
  test('General section shows Notifications toggle', async ({ page }) => {
    const generalBtn = page.locator('main').getByRole('button', { name: /^general$/i }).first();
  ...
  test('Enabling Developer mode shows Developer section in nav', async ({ page }) => {
    ...
    const devToggle = page.getByText(/developer mode/i).locator('..').locator('..').getByRole('button').last();
    if (await devToggle.count() === 0) { test.skip(); return; }
  ...
  test('Downloads section shows max concurrent downloads control', async ({ page }) => {
    const downloadsBtn = page.locator('main').getByRole('button', { name: /^downloads$/i }).first();
  ...
  test('Updates section shows app version', async ({ page }) => {
    const updatesBtn = page.locator('main').getByRole('button', { name: /updates/i }).first();
  ...
  test('Check now button calls check-for-updates IPC', async ({ page, electronApp }) => {
    ...
    if (await checkBtn.count() === 0) { test.skip(); return; }
  ...
  test('Credits section shows contributor info', async ({ page }) => {
    const creditsBtn = page.locator('main').getByRole('button', { name: /credits/i }).first();
    if (await creditsBtn.count() === 0) { test.skip(); return; }
