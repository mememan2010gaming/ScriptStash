...
test.describe('Sidebar navigation', () => {
  ...
  test('sidebar contains all nav items', async ({ page }) => {
    ...
    await expect(nav.getByText(/free scripts|paid scripts/i).first()).toBeVisible({ timeout: 8_000 });
  ...
  test('can switch between views multiple times', async ({ page }) => {
    ...
    await expect(page.locator('main').getByText(/downloads/i).first()).toBeVisible({ timeout: 8_000 });
    await app.goToSettings();
    await expect(page.getByText(/settings|download path|theme/i).first()).toBeVisible({ timeout: 8_000 });
    await app.goToTopics();
