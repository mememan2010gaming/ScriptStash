...
class SettingsPage extends AppPage {
  ...
  section(name) {
    return this.page
      .locator('[class*="section"], [class*="group"], [class*="panel"]')
      .filter({ hasText: name })
      .first()
  }
  ...
  downloadPathDisplay() {
    return this.page
      .locator('input[readonly], [class*="path"]')
  ...
  maxDownloadsInput() {
    return this.page.getByRole('spinbutton').or(this.page.locator('input[type="number"]')).first()
  }
  ...
  ytdlpVersionDisplay() {
    return this.page
      .getByText(/yt-dlp/i)
      .locator('..')
      .locator('[class*="version"], code, span')
      .first()
  }
