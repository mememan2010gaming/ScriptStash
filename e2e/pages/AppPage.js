...
class AppPage {
  ...
  sidebar() { return this.page.locator('aside').first(); }

  async navigateTo(label) {
    const sidebar = this.sidebar();
    const link = sidebar.getByRole('button', { name: label, exact: false })
  ...
  async goToTopics()    { await this.navigateTo('Free Scripts'); }
  async goToSearch()    { await this.navigateTo('Search'); }
  async goToDownloads() { await this.navigateTo('Downloads'); }
  async goToSettings()  { await this.navigateTo('Settings'); }
  async goToNotifications() { await this.navigateTo('Notifications'); }

  // ─── Titlebar ───────────────────────────────────────────────────────────────

  // The custom titlebar has no class containing "titlebar"; target its Minimize button instead.
  minimizeBtn() { return this.page.getByRole('button', { name: /minimiz/i }).first(); }
  maximizeBtn() { return this.page.getByRole('button', { name: /maximiz/i }).first(); }

  async minimize() { await this.minimizeBtn().click(); }
  async maximize() { await this.maximizeBtn().click(); }
