...
class DownloadsPage extends AppPage {
  ...
  activeItems() {
    return this.page.locator('[class*="active"], [class*="Active"]')
      .filter({ hasText: /\d+%|\bkB\b|\bMB\b|downloading/i });
  }

  progressBar(index = 0) {
    return this.page.locator('[role="progressbar"], [class*="progress"], [class*="Progress"]').nth(index);
  }
  ...
  historyItems() {
    return this.page.locator('[class*="history"], [class*="completed"], [class*="item"]')
      .filter({ hasText: /.funscript|.mp4|download/i });
  }

  async historyCount() { return this.historyItems().count(); }
  ...
  async cancelDownload(index = 0) {
    ...
    await item.getByRole('button', { name: /cancel|stop|remove/i }).first().click();
  }

  // ─── Empty state ─────────────────────────────────────────────────────────────

  emptyState() {
    return this.page.getByText(/no downloads/i)
