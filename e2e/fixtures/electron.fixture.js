...
const MOCK_IPC = {
  topics: {
    success: true,
    data: { topics: MOCK_TOPICS, total: MOCK_TOPICS.length, page: 1, perPage: 20 },
  },
  ...
  notifications: {
    ...
    data: {
      ...
      total_rows_notifications: MOCK_NOTIFICATIONS.length,
    },
  },
...
async function injectMocks(electronApp, mocks) {
  await electronApp.evaluate(({ ipcMain }, m) => {
    ...
    replace('stream-video', () => ({
      success: true,
      data: { streamUrl: 'http://localhost:9999/stream/test' },
    }))
    ...
    // Update
    replace('check-for-updates', () => ({
      success: true,
      data: { updateAvailable: false, current: '2.4.29' },
    }))
